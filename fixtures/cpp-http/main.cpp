...
extern "C" void exports_wasi_http_incoming_handler_handle(
  ...
  if (has_path && path.len >= 5 && __builtin_memcmp(path.ptr, "/echo", 5) == 0) {
    ...
    if (wasi_http_types_method_incoming_request_consume(
      ...
      wasi_io_streams_own_input_stream_t input{};
  ...
  wasi_http_types_static_outgoing_body_finish(body, nullptr, &finish_error);
